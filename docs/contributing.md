...
<!--
This section is outdated due to recent refactoring of the gLayout repo structure.
The layout and testbench integration workflow is being updated.
-->

We are currently revising how blocks and circuit generators are structured. For now, contributors are encouraged to:
- Place new components under `glayout/flow/blocks/<your_block>/`
- Provide `.py` layout scripts and corresponding `.spice` netlists
- Ensure DRC & LVS clean results (see "Checks" section below)
- Document your block with a `README.md` including layout images and parameters
...
## Parametric Simulations (Deprecated / WIP)
<!--
...
## CI Checks ⚙️ (Not yet implemented in current gLayout repo)

While CI pipelines are planned (e.g., GitHub Actions), they are not currently enabled in this repo.

Contributors are still encouraged to:
- Run Magic / KLayout DRC
- Run Netgen LVS
- Provide clean layout/netlist artifacts for their blocks
